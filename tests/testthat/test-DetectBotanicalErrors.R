test_that("DetectBotanicalErrors", {
  ...
  Rslt <- DetectBotanicalErrors(Data)

    # No correction, only comments
    expect_true(all(!(grepl("_TreeDataCor", names(Rslt)))) & "Comment" %in% names(Rslt))

    # Missing value
    Rslt[is.na(Subspecies), Subspecies := ""] # Subspecies = NA is ok
    expect_true(all(grepl("Missing value", Rslt[rowSums(is.na(Rslt)) > 0, Comment])))

    # -aceae in Genus or Species
    expect_true(all(grepl("'aceae' cannot be genus or species names",
                          Rslt[grepl("aceae", Data$Genus) | grepl("aceae", Data$Species), Comment])))

    # Special character in Genus, Family
    expect_true(all(grepl("Special characters",
                          Rslt[grepl('[[:punct:]]', Data$Genus) | grepl('[[:punct:]]', Data$Family), Comment])))

    # Variant botanical info per IdTree (A FAIRE)
    VarIdTree <- unique(Rslt[rowSums(is.na(Rslt)) == 0, .(IdTree, Family, Genus, Species, Subspecies, VernName)])[duplicated(IdTree), IdTree]

    expect_true(all(grepl("Different botanical informations",
                          Rslt[IdTree %in% VarIdTree, Comment])))
